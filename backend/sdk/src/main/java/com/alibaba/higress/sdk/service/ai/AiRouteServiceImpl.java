...
public class AiRouteServiceImpl implements AiRouteService {
    ...
    public AiRoute add(AiRoute route) {
        ...
        // Trigger related providers to update their matchRules from pure SERVICE to ROUTE+SERVICE
        syncRelatedProviders(route);
    ...
    public AiRoute update(AiRoute route) {
        ...
        // Read the old route before overwriting, so we can detect removed providers
        AiRoute oldRoute = query(route.getName());
        Set<String> oldProviderNames = collectProviderNames(oldRoute);
        ...
        // Clean up orphaned ROUTE+SERVICE matchRules for providers that were removed from this route
        Set<String> newProviderNames = collectProviderNames(route);
        String routeResourceName = buildRouteResourceName(route.getName());
        for (String removedProvider : oldProviderNames) {
            if (newProviderNames.contains(removedProvider)) {
                continue;
            }
            try {
                UpstreamService upstreamService = llmProviderService.buildUpstreamService(removedProvider);
                ...
                log.warn("Failed to clean up orphaned matchRule for removed provider: {}", removedProvider, e);
            }
        }

        // Trigger related providers to update their matchRules
        syncRelatedProviders(route);
    ...
    private static Set<String> collectProviderNames(AiRoute route) {
        ...
        if (CollectionUtils.isNotEmpty(route.getUpstreams())) {
            for (AiUpstream upstream : route.getUpstreams()) {
                if (StringUtils.isNotEmpty(upstream.getProvider())) {
                    names.add(upstream.getProvider());
                }
            }
        }
        ...
        if (fallbackConfig != null && Boolean.TRUE.equals(fallbackConfig.getEnabled())
            && CollectionUtils.isNotEmpty(fallbackConfig.getUpstreams())) {
            for (AiUpstream upstream : fallbackConfig.getUpstreams()) {
                if (StringUtils.isNotEmpty(upstream.getProvider())) {
                    names.add(upstream.getProvider());
                }
            }
    ...
    /**
     * Trigger related providers to update their matchRules.
     * When an AiRoute is created or updated, providers using this route should upgrade
     * their matchRules from pure SERVICE dimension to ROUTE+SERVICE combination.
     */
    private void syncRelatedProviders(AiRoute route) {
        if (CollectionUtils.isEmpty(route.getUpstreams())) {
            return;
        }

        // Collect all provider names from upstreams and fallback upstreams
        Set<String> providerNames = new HashSet<>();
        for (AiUpstream upstream : route.getUpstreams()) {
            if (StringUtils.isNotEmpty(upstream.getProvider())) {
                providerNames.add(upstream.getProvider());
            }
        }
        ...
        // Trigger each provider to update
        for (String providerName : providerNames) {
            try {
                LlmProvider provider = llmProviderService.query(providerName);
                if (provider != null) {
                    llmProviderService.addOrUpdate(provider);
                }
            } catch (Exception e) {
                // Log but don't fail the route creation/update
                // The provider update can be retried later
            }
        }
    }
